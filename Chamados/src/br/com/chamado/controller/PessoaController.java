	...
	public String senha() {
		try {
			...
			for (Pessoa pessoa1 : listaPessoa) {
				...
				if(pessoaBean.getNome().equals(pessoa.getNome()) && pessoaBean.getSenha().equals(pessoa.getSenha())){
					FacesMessage msg = new FacesMessage("Senha Correta");
					getFacesContext().addMessage(null, msg);
					return "principal";
				}else{
					FacesMessage msg = new FacesMessage("Usuario ou Senha Incorretos");
			...
			String msg = "Listagem não realizada. Movito: "
					+ ((e instanceof ChamadoException ? ((ChamadoException) e).getEx().getMessage() : ""));
			FacesMessage message = new FacesMessage(msg);
